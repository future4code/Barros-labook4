...
export class PostDatabase extends BaseDatabase {
  ...
  feedPost = async (input: string[]): Promise<FeedPostDBDTO[]> => {
    try {
      const [result] = await PostDatabase.connection.raw(
        `select * from ${PostDatabase.TABLE_NAME} where author_id in (${input}) order by created_at;`
      );
      return result;
    } catch (error: any) {
      throw new Error(error.message);
  ...
  feedPostAll = async (input: PostTypeDTO): Promise<FeedPostDBDTO[]> => {
    try {
      const [result] = await PostDatabase.connection.raw(
        `select * from ${PostDatabase.TABLE_NAME} where type = "${input.type}" order by created_at;`
      );
      return result;
    } catch (error: any) {
      throw new Error(error.message);
